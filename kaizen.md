...
- Hypothesis: make freeze-commit receipt lookup robust to quiet commits, which suppress the printed SHA, and to amended or missing SHAs. In this run, `fidelity freeze commit is missing or not ancestral to task HEAD` blocked integration. Evaluate extraction from the tree/history before changing the gate. Reassess when receipt lookup or the gate changes.
- Hypothesis: make the re-gate path after a test-only commit discoverable from current receipts. Here, `regate-passed` required an eye whose `reviewed_head_sha` equaled HEAD; this was obtained by querying `harness_reviews` with affected roles and the reason, then dispatching only those roles. Reassess when re-gate receipts or review dispatch guidance changes.
