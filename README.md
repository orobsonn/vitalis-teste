...
As [fontes fornecidas](docs/fontes/) são o [CSV de 80 guias](docs/fontes/guias.csv), o [dicionário](docs/fontes/dicionario_dados.md) e o [JSON versionado dos convênios](docs/fontes/regras_convenio.json). O [PRD](docs/prd/vitalis-conferencia-preventiva-guias.md) registra as decisões de produto. As respostas do recrutador esclareceram especialmente que **não há regra complementar além do JSON** e que não devemos preencher lacunas com supostas normas clínicas ou contratuais.
